...
from ai.ai_util import clamp_image, head_from_bounds, AbstractElement, get_neg_pos_comp
...
class DeepPoly_f(AbstractElement):
    ...
    @classmethod
    def construct_from_noise(cls, x: Tensor, eps: Union[float,Tensor], data_range: Tuple[float,float]=None, dtype:Optional[str]=None) -> "DeepPoly_f":
    ...
    @classmethod
    def construct_constant(cls, x: Tensor, inputs:Optional["HybridZonotope"]=None, dtype:Optional[str]=None) -> "DeepPoly_f":
        # compute center and error terms from input, perturbation size and data range
        if dtype is None:
            dtype = x.dtype
    ...
    @classmethod
    def construct_from_bounds(cls, min_x: Tensor, max_x: Tensor, dtype: torch.dtype=torch.float32) -> "DeepPoly_f":
    ...
    def contains(self, other: "DeepPoly_f") -> bool:
        ...
        def add_input_encoding_to_model(inputs: "HybridZonotope", model: "Model", var_list: list, counter: int) -> Tuple["Model", list, int, int]:
            assert inputs.shape[0] == 1 # only one batch element

            flat_inputs_self = inputs.flatten()[0]
            num_pixels = inputs.head.numel()
            num_error_terms = 0 if self.inputs.errors is None else self.inputs.errors.shape[0]
            for j in range(num_error_terms):
                var_name = "e_" + str(j + counter)
                var = model.addVar(vtype=GRB.CONTINUOUS, lb=-1, ub=1, name=var_name)
                var_list.append(var)
            counter += num_error_terms
            lb, ub = flat_inputs_self.concretize()
            if flat_inputs_self.beta is not None:
                for i in range(num_pixels):
                    var_name = "e_" + str(counter + i)
                    var = model.addVar(vtype=GRB.CONTINUOUS, lb=-1, ub=1, name=var_name)
                    var_list.append(var)
                counter += num_pixels
            ...
            for i in range(num_pixels):
                ...
                expr += 1 * var_list[num_error_terms + i]   # box error component
                if flat_inputs_self.errors is not None:
                    expr.addTerms(flat_inputs_self.errors[:,i], var_list[:num_error_terms+1]) # zonotope terms
                expr.addConstant(flat_inputs_self.head[i])
                model.addConstr(expr, GRB.EQUAL, 0)
            counter += num_pixels

            return model, var_list, counter, counter - num_pixels, num_pixels

        model, var_list, counter, self_d0_in_s, inputs_dim_s = add_input_encoding_to_model(self.inputs, model, var_list, counter)

        if self.inputs != other.inputs:
            model, var_list, counter, self_d0_in_o, inputs_dim_o = add_input_encoding_to_model(other.inputs, model, var_list, counter)
        else:
            self_d0_in_o = self_d0_in_s
            inputs_dim_o = inputs_dim_s

        num_pixels = self.x_l_bias.numel()
        ...
        for i in range(num_pixels):
            ...
            var_list.append(var)
            lb_expr = LinExpr()
            lb_expr.addTerms(flat_self.x_l_coef[:, i], var_list[self_d0_in_s:self_d0_in_s+inputs_dim_s])
            lb_expr.addConstant(flat_self.x_l_bias[i])
            model.addConstr(lb_expr, GRB.LESS_EQUAL, var)
            ub_expr = LinExpr()
            ub_expr.addTerms(flat_self.x_u_coef[:, i], var_list[self_d0_in_s:self_d0_in_s+inputs_dim_s])
        ...
        for i in range(num_pixels):
            obj = LinExpr()
            obj.addTerms(flat_other.x_l_coef[:, i], var_list[self_d0_in_o:self_d0_in_o + inputs_dim_o])
            obj -= 1 * var_list[counter + i]
            lb_expr.addConstant(flat_other.x_l_bias[i])
            ...
            obj = LinExpr()
            obj.addTerms(-flat_other.x_u_coef[:, i], var_list[self_d0_in_o:self_d0_in_o + inputs_dim_o])
            obj += 1 * var_list[counter + i]
            lb_expr.addConstant(-flat_other.x_u_bias[i])
            model.setObjective(obj, GRB.MAXIMIZE)
            model.update()
            model.reset()
            model.optimize()
    ...
    def is_greater(self, i:int, j:int, threshold_min:Union[Tensor,float]=0) -> Tuple[Tensor,bool]:
        input_lb, input_ub = self.inputs.concretize()
        input_lb, input_ub = input_lb.unsqueeze(dim=0), input_ub.unsqueeze(dim=0)
