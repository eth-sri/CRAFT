...
import monDEQ.utils as utils
import time

"""
Based on: https://github.com/locuslab/monotone_op_net/blob/master/splitting.py
"""

class MONForwardBackwardSplitting(nn.Module):

    def __init__(self, linear_module, nonlin_module, alpha=1.0, tol=1e-7, max_iter=50, verbose=False):
        super().__init__()
    ...
    def forward_step(self, x, z , alpha, deepz_lambdas=None):
        """ Do a single forward step """
        z_n = self.linear_module._forward_abs(x, z)
        z_n = z * (1 - alpha) + z_n * alpha
        assert (z_n.concretize()[1] - z_n.concretize()[0] >= 0).all()
        z_n = self.nonlin_module._forward_abs(z_n, deepz_lambdas)
        return z_n
...
class MONPeacemanRachford(nn.Module):
    ...
    def forward_step(self, x, z, u, alpha, deepz_lambdas=None, Uxb=None):
        """ Do a single forward step """
        if Uxb is None:
            Uxb = x.linear(self.linear_module.U.weight, self.linear_module.U.bias)

        u_12 = z * 2. - u
        p_12 = u_12 + Uxb * alpha
        if len(p_12.shape) > 2:
            z_12 = p_12.fft_conv(self.linear_module.Winv)
        else:
            z_12 = p_12.linear(self.linear_module.Winv, bias=None)
        u_n = z_12 * 2. - u_12
        z_n = self.nonlin_module._forward_abs(u_n, deepz_lambdas)
        return z_n, u_n
        ...
        with torch.no_grad():
            ...
            while (err > self.tol and it < self.max_iter):
                ...
                if self.save_abs_err:
                    ...
                    err = sum((zn[i] - z[i]).norm().item() / (1e-7 + zn[i].norm().item()) for i in range(n))
                z = zn
                it = it + 1
